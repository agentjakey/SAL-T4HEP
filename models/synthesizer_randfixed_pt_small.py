...
# ---------------------------
class RandomSynthesizerAttention(layers.Layer):
    ...
    def build(self, input_shape):
        # Fixed random attention scores, not trainable
        initializer = tf.keras.initializers.RandomNormal(mean=0.0, stddev=1.0)
        self.fixed_scores = self.add_weight(
            shape=(1, 1, self.seq_len, self.seq_len),
            initializer=initializer,
            trainable=False,
            name="fixed_random_scores"
        )
    ...
    def call(self, x, training=None):
        ...
        attn_weights = tf.nn.softmax(self.fixed_scores, axis=-1)  # [1, 1, L, L]
        attn_weights = tf.tile(attn_weights, [batch_size, self.num_heads, 1, 1])  # [B, h, L, L]
...
class SynthesizerTransformerBlock(layers.Layer):
    def __init__(self, d_model, d_ff, output_dim, num_heads, seq_len, **kwargs):
        super().__init__(**kwargs)
        self.attn = RandomSynthesizerAttention(d_model, num_heads, seq_len)
