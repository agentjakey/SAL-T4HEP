...
model_name = 'randlearned_pt_small'
...
# ---------------------------
# Random Learned Synthesizer Attention Layer 
# ---------------------------
class RandomLearnedSynthesizerAttention(layers.Layer):
    """
    Random-Learned Synthesizer: learn a global (input-independent) attention map per head.
    Shape: learned_scores = [1, num_heads, L, L]; softmax over last dim.
    """
    def __init__(self, d_model, num_heads, seq_len, **kwargs):
    ...
    def build(self, input_shape):
        # Trainable attention logits (random init), one [L x L] per head
        init = tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.02)
        self.learned_scores = self.add_weight(
            name="learned_random_scores",
            shape=(1, self.num_heads, self.seq_len, self.seq_len),
            initializer=init,
            trainable=True,
        )
        # Value projection (same as your file)
    ...
    def call(self, x, training=None):
        B = tf.shape(x)[0]
        v = tf.matmul(x, self.value_proj)       # [B, L, D]
        v = self._split_heads(v, B)             # [B, h, L, d]

        # Broadcast learned per-head map across the batch
        scores = tf.tile(self.learned_scores, [B, 1, 1, 1])  # [B, h, L, L]
...
class SynthesizerTransformerBlock(layers.Layer):
    def __init__(self, d_model, d_ff, output_dim, num_heads, seq_len, **kwargs):
        super().__init__(**kwargs)
        self.attn = RandomLearnedSynthesizerAttention(d_model, num_heads, seq_len)
...
def build_synthesizer_transformer_classifier(
    ...
    x = SynthesizerTransformerBlock(
        d_model = d_model, d_ff = d_ff, output_dim = output_dim, num_heads = num_heads, seq_len = num_particles
    )(x) 
...
# Load data
X_train = np.load("/j-jepa-vol/l1-jet-id/data/jetid/processed/x_train_robust_150const_ptetaphi_sorted_pt.npy")
...
X_test = apply_sorting(X_test, sort_by="pt")
